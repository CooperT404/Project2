...
class DungeonMasterServer:
    def __init__(self, game_log, dm_hook=lambda : '', host="localhost", port=5555, countdown=10):
        self.host = host
        self.port = port
    ...
    def accept_clients(self):
     while self.running:
        try:
            ...
            # If the server_socket is closed, exit the loop gracefully.
            if not self.running:
                break
            else:
                # If it happened unexpectedly while still running, you might log it
                print(f"[DM] Accept error: {e}")
                break
    ...
    def game_loop(self):
        ...
        while self.running:
            ...
            self.broadcast(b"Please enter /combat to start the fight or '/quit' to leave\n")
